...
internal static class AppIO
{
    ...
    {
        return AnsiConsole.Confirm($"[yellow]{Markup.Escape(message)}[/]", defaultValue: false);
    }

    public static T WithStatus<T>(string statusMessage, Func<T> action)
    {
    ...
    public static Task<T> WithStatusAsync<T>(string statusMessage, Func<Task<T>> action)
    {
        if(!AnsiConsole.Profile.Capabilities.Interactive)
        {
            return action();
    ...
    public static Task TryDeleteDirectoryAsync(string path)
    {
        return Task.Run(() => TryDeleteDirectory(path));
    }
}
